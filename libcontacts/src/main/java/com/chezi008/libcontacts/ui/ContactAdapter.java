...
//        }
        //头像就放在avatar里面，存放的是本地的头像的完整路径
        if(contactBean.getAvatar().equals("null")){
            contactAdapterHolder.ivAvatar.setImageResource(R.drawable.ic_default_avatar);
        }else{
            contactAdapterHolder.ivAvatar.setImageBitmap(BitmapFactory.decodeFile(contactBean.getAvatar()));
        }
